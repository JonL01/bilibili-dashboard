...
def generate_insights():
    ...
    def _llm_angles(title, summary, like_rate, coin_rate, fav_rate, share_rate, reply_rate, retry=2):
        ...
        prompt = (
            ...
            "格式参考：🔥 情绪共鸣型 | 视频的标题里的具体内容引发全网共鸣 | 聚焦此视频的细节，解释情感冲击与高互动的原因。\n"
            "要求：\n"
            "- 标题必须引用此视频摘要中的具体细节，不能用通用标题\n"
            "- 正文1-2句，不超过60字\n"
            "- 不要照抄格式参考示例"
        )
    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        pool.map(generate_video_angles, [h for h in hot_insights if h.get("summary")])
    for v in hot_insights:
        if "angles" not in v:
            v["angles"] = generate_angles(
                v, v["like_rate"], v["coin_rate"], v["share_rate"],
